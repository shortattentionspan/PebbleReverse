...
const GPathInfo HOUR_HAND_POINTS = { 4, (GPoint[] ) { { -5, 12 }, { 5, 12 }, { 5, -43 }, { -5, -43 }, } };
// Date Window
...
void draw_hands(Layer *layer, GContext *ctx){
  ...
  time_t now = time(NULL);
	...
	gpath_rotate_to(hour_hand, /*TRIG_MAX_ANGLE*/ -( (((TRIG_MAX_ANGLE * t->tm_hour) / 12) + (((TRIG_MAX_ANGLE * t->tm_min) / 60) / 12)))); //Moves hour hand 1/60th between hour indicators.  Only updates per minute.
	gpath_draw_filled(ctx, hour_hand);
	gpath_draw_outline(ctx, hour_hand);
	// Minute hand
	gpath_rotate_to(minute_hand, /*TRIG_MAX_ANGLE*/ - (TRIG_MAX_ANGLE * t->tm_min / 60));
...
void init() {
	...
	date_window_layer = bitmap_layer_create(GRect(117,75,28,22));
	...
	date_layer = text_layer_create(GRect(119, 72, 25, 30));
